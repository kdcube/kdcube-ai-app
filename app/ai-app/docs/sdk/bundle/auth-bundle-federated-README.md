...
The token issuer and Socket.IO verifier must use the same
`KDCUBE_FEDERATED_TOKEN_SECRET`.

In the normal local CLI runtime this is generated by the CLI and injected into
both `chat-ingress` and `chat-proc`. In managed deployments it must be a shared
runtime secret injected into every component that can issue or verify
federated Data Bus tokens.
