...
Before selecting SDK blocks or writing code, write one surface map:

- **Provides (`as_provider`):** what users, clients, agents, or other apps may
  invoke or mount; entrypoint decorators declare the surface and
  `surfaces.as_provider` owns its provider policy.
- **Consumes (`as_consumer`):** which tools, skills, MCP servers, named services,
  or connected capabilities this app and each agent may use.

List only the surfaces the app actually has: chat/agent, REST, MCP, named
services, widgets/UI, Data Bus, external events, cron/jobs, and files. This map
drives decorators, SDK blocks, auth, config/secrets, interface docs, economics,
and tests. An app may be MCP-only, API-only, widget-only, or backend-only.

### MCP is two independent surface decisions

"Uses MCP" is incomplete. Record either or both directions:

- **Consume MCP:** register the server once under
  `surfaces.as_consumer.mcp.services.mcpServers`, then give each agent an
...
The directions are independent. Connecting a server does not require `@mcp`;
...
| ReAct agent with bundle tools and skills | `BaseWorkflow.build_react(...)`, `surfaces.as_consumer.agents.<agent>.tools`, `surfaces.as_consumer.agents.<agent>.skills`; skills are discovered from core SDK, SDK solution roots, and bundle `custom_root`, then filtered by exact consumer id; per-agent react config (`config.react.<agent>`: instructions, `supported_models`, budgets) resolves through the agent-key chain | [How To Construct A ReAct Agent](../../agents/react/how/how-to-construct-react-agent-README.md), [Bundle Agent Integration](../bundle-agent-integration-README.md) |
| Full chat surface (timeline, composer, consent banners, "+" menu) served from your app and wired to your agent | `chat_widget_ui_config()` mapping `config.ui.widgets.<alias>` to `sdk://solutions/chat/ui/widget`, `engine: local\|package\|package-ui` | [Chat Widget Solution](../../solutions/chat/chat-widget-solution-README.md), [Chat Widget recipe](../../../recipes/components/chat-README.md) |
| Conversation-scoped agent customization (tools/skills/MCP/namespace operation-action toggles + model pick) | `agent_capabilities(conversation_id)` / `agent_selection_update(conversation_id)` on the SDK entrypoint base + `apply_user_agent_selection` in the workflow; chat edits remain a draft until **Save changes**. The same picker body also powers the served `capabilities` widget (`sdk://solutions/chat/ui/widget-capabilities`), whose unscoped mode manages the baseline for future conversations | [Conversation-Scoped Agent Capabilities](../../solutions/user-settings/capabilities-README.md), [How To Construct A ReAct Agent](../../agents/react/how/how-to-construct-react-agent-README.md) |
| Per-role model routing and temporary model strength selection | `config.role_models` for defaults/descriptor overrides; `bundle_call_context.role_models` for one API/MCP/cron/chat/job call | [Bundle Agent Integration](../bundle-agent-integration-README.md#2a-model-selection-for-agent-roles), [Bundle Runtime](../bundle-runtime-README.md#request-scoped-role-model-override) |
...
| App scene that connects chat, canvas, namespace objects, widgets, and live client events into one workspace | scene host + `@kdcube/components-core/scene` style runtime contract: surface registry, config handshake, namespace presentation relay, context drag broker, provider-backed open routing, and optional event-bus/Data Bus relay; the component set is server-configured (`ui.scene.components`, merged by alias over code defaults) and an entry can mount ANOTHER app's served widget | [Scene recipe](../../../recipes/components/scene-README.md), [Scene Configuration Examples](../../solutions/scene/config/README.md), [Scene Composition](../../solutions/scene/scene-composition-README.md), [Scene Surface Registry](../../solutions/scene/scene-surface-registry-README.md), [Cross-Surface Context Drag](../../solutions/scene/cross-surface-context-drag-README.md), [Scene Event Orchestration](../../solutions/scene/scene-event-orchestration-README.md) |
| One mounted widget summons and directs another (declare contract, receive + ack, emit with fallback) | `kdcube.surface.command` + `command_id` ack; contract declared on the component spec (`target_surfaces`) or scene config | [Scene Surface Command recipe](../../../recipes/components/scene-surface-command-README.md), [Scene Surface Commands](../../solutions/scene/scene-surface-commands-README.md) |
...
| Expose a namespace (`task:`, …) of objects/actions for other bundles to call, **or** consume another bundle's namespace as canvas pins / chat chips / agent tools | Owner side: `named_service` API operation + `NamedServiceRegistry`, including the realm's PRESENTATION layer (purpose, works-with, per-operation/action labels) and connected-account requirements — the self-description feeds both the agent catalog and the user-facing service card. Consumer side: `named_services.namespaces.<ns>.provider` config + generic resolver — no shared code, configured discovery | [Namespace Services](../../namespace-services/README.md) (index → Providers / Clients / Integration), [Providers](../../namespace-services/providers-README.md), [Bundle Subsystem Integration](../bundle-subsystem-integration-README.md) |
...
| Live events from a non-chat widget/API operation to the browser | `/sse/stream` or Socket.IO plus `KDC-Stream-ID`; bundle emits `comm.service_event(...)` from request-bound context | [Client Transport Protocols](../../../service/comm/client-transport-protocols-README.md#non-chat-app-events-over-the-shared-stream), [Bundle Transports](../bundle-transports-README.md#71-communicator-output) |
...
- declare the component set server-side under `ui.scene.components` — entries
  merge by alias over the scene's code defaults and can mount another app's
  served widget (see the
  [Scene recipe](../../../recipes/components/scene-README.md#scene-components));
