...
| Per-user agent customization (tools/skills/MCP/namespace operation-action toggles + model pick) | `agent_capabilities` / `agent_selection_update` on the SDK entrypoint base + `apply_user_agent_selection` in the workflow; one picker body in three shells — the chat composer "+" popover, an expanded in-chat modal, and the served `capabilities` widget (`sdk://solutions/chat/ui/widget-capabilities`) a scene can mount and summon via `capabilities.open` | [Per-User Agent Capabilities](../../solutions/user-settings/capabilities-README.md), [How To Construct A ReAct Agent](../../agents/react/how/how-to-construct-react-agent-README.md) |
