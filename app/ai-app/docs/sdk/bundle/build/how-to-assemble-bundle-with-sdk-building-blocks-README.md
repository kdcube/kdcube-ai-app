...
| Per-user agent customization (tools/skills/MCP/namespaces toggles + model pick) in the chat composer | `agent_capabilities` / `agent_selection_update` on the SDK entrypoint base + `apply_user_agent_selection` in the workflow; the chat component's "+" menu consumes them | [How To Construct A ReAct Agent](../../agents/react/how/how-to-construct-react-agent-README.md) |
