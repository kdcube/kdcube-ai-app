...
| `surfaces.as_provider` | integration bridge and Bundle Admin descriptors | `<prefix>/bundles/<bundle_id>/descriptor` `config.surfaces.as_provider` | `bundles.yaml -> items[].config.surfaces.as_provider` | cache | provider-side surface visibility and authority/grant policy |
...
| `surfaces.as_consumer.mcp.services` | `self.bundle_prop("surfaces.as_consumer.mcp.services")` | `<prefix>/bundles/<bundle_id>/descriptor` `config.surfaces.as_consumer.mcp.services` | `bundles.yaml -> items[].config.surfaces.as_consumer.mcp.services` | cache | canonical MCP transport/auth config |
| `mcp.services` | `self.bundle_prop("mcp.services")` | `<prefix>/bundles/<bundle_id>/descriptor` `config.mcp.services` | `bundles.yaml -> items[].config.mcp.services` | cache | legacy MCP transport/auth fallback |
...
| `surfaces.as_consumer.mcp.services` | yes | platform runtime | app props authority + Redis cache |
...
| `namespace_styles` | no, bundle-owned namespace presentation | namespace-aware surfaces such as chat context chips, search results, canvas cards, and scene drag/drop overlays | same bundle props storage as any other non-secret prop; exposed to clients through the app's public `namespace_presentation_config` API |
...
This property is reserved for MCP connector configuration consumed by app
agents.
...
Behavior:
- `surfaces.as_consumer.mcp.services.mcpServers` and
  `surfaces.as_consumer.mcp.services.servers` are both accepted.
...
- for app-local MCP config, prefer:
...
- top-level `mcp.services` and `MCP_SERVICES` env are still accepted only as
  legacy/local-dev fallbacks when `surfaces.as_consumer.mcp.services` is not
  configured in app props.
...
- `surfaces.as_consumer.mcp.services` controls how those MCP servers are
...
| Question | Answer |
|---|---|
| Where do I set MCP server URLs/auth? | `config.surfaces.as_consumer.mcp.services` in app props |
| Where should auth secrets go? | bundle or platform secrets via `get_secret(...)`, not inside plain props |
| What gets exported by CLI? | the non-secret `surfaces.as_consumer.mcp.services` config, not the resolved secret values |
...
`surfaces.as_consumer.mcp.services` through the live admin props API in an `aws-sm` deployment, this
