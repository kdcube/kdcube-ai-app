...
|---|---|---|---|---|---|---|---|---|
| 1 | User prompt event projection | Lane fold in `browser.py` delegates to built-in block-production policies. | Not applicable. | Accepted type is `event.user.prompt`; `react.block_production.user_prompt_default` emits `user.prompt`. | Existing renderer handles visible text at the `ar:<turn>.user.prompt...` path. | Current user context can be mentioned separately. | Existing summary selection includes it. | Implemented built-in user event source. |
...
| `react.write` | `tool_call_id` | Native `react.tool.call` / `react.tool.result` / `react.note` / `fi:` file blocks | Native handler. | Native handler writes blocks directly. | Identity policy. | Pending. | Identity policy; some compaction preservation remains hardcoded. |
