...
| `react.rg` local search | Search materialized local OUT_DIR files, not hidden timeline or unpulled history. | Workspace guide plus path guidance. | `REACT_LITE_LOCAL_ARTIFACT_SEARCH`; `workspace`, `workspace_exec`, `document`, `web`, `all_capabilities`. |
...
| Workspace mental model | React uses timeline/logical paths plus current-turn OUT_DIR, not arbitrary host fs. | `get_workspace_implementation_guide(...)`, exec/path guidance. | `REACT_LITE_WORKSPACE_BASE`; all profiles. |
| OUT_DIR tree | Physical materialized shape for exec/code: current turn, pulled older turns, logs. | Workspace guide, `EXEC_SNIPPET_RULES`. | `REACT_LITE_WORKSPACE_BASE`; all profiles. |
