...
| Variable | Purpose | File | Service/Scope |
|---|---|---|---|
...
| `POSTGRES_DATABASE` | Postgres database name | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/kb/worker/postgres |
| `POSTGRES_PORT` | Postgres port | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/kb/worker/postgres |
| `POSTGRES_SSL` | Enable SSL for Postgres | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/kb/worker |
| `POSTGRES_DB` | Postgres container DB name | `deployment/docker/all_in_one/sample_env/.env.backend` | postgres container |
| `PGPORT` | Postgres port for client tools | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/kb/worker |
| `SELF_HOSTED_SERVING_ENDPOINT` | Custom model serving endpoint | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/kb/worker |
...
| `KDCUBE_STORAGE_PATH` | Storage backend path or S3 URI | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/kb/worker |
| `ORCHESTRATOR_WORKER_CONCURRENCY` | Worker concurrency for orchestrator | `deployment/docker/all_in_one/sample_env/.env.backend` | worker/orchestrator |
| `CB_ORCHESTRATOR_TYPE` | Orchestrator name | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/worker |
| `CB_RELAY_IDENTITY` | Redis pubsub identity | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/worker |
| `DRAMATIQ_PROCESSES` | Dramatiq worker process count | `deployment/docker/all_in_one/sample_env/.env.backend` | worker |
| `MAX_QUEUE_SIZE` | Hard cap for enqueue | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
...
| `GEMINI_CACHE_ENABLED` | Enable Gemini cache | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/kb |
| `GEMINI_CACHE_TTL_SECONDS` | Gemini cache TTL | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/kb |
| `APP_DOMAIN` | CORS allow domain | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
| `TORCH_DEVICE` | Marker torch device (CPU/GPU) | `deployment/docker/all_in_one/sample_env/.env.backend` | kb |
| `AUTH_PROVIDER` | Auth provider (simple|cognito) | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
| `EXTRA_ID_TOKEN_HEADER` | Extra ID token header name | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
| `COGNITO_REGION` | Cognito region | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
...
| `JWKS_CACHE_TTL_SECONDS` | JWKS cache TTL | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
| `OIDC_SERVICE_ADMIN_USERNAME` | OIDC service admin username | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
| `OIDC_SERVICE_ADMIN_PASSWORD` | OIDC service admin password | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
| `ODIC_SERVICE_USER_EMAIL` | Service user email | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
| `AGENTIC_BUNDLES_ROOT` | Bundles root inside container | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/kb/worker |
...
| `DEFAULT_LLM_MODEL_ID` | Default LLM model | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/kb |
| `DEFAULT_EMBEDDING_MODEL_ID` | Default embedding model | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/kb |
| `APP_AV_SCAN` | Enable AV scan | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
| `APP_AV_TIMEOUT_S` | AV scan timeout | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
| `CLAMAV_HOST` | ClamAV host | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
| `CLAMAV_PORT` | ClamAV port | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
| `AWS_REGION` | AWS region | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/kb |
| `AWS_DEFAULT_REGION` | AWS default region | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/kb |
| `NO_PROXY` | No-proxy hosts | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/kb |
| `AWS_EC2_METADATA_DISABLED` | Allow EC2 IMDS | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/kb |
| `AWS_SDK_LOAD_CONFIG` | Load AWS config file | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/kb |
| `TOOLS_WEB_SEARCH_FETCH_CONTENT` | Enable web fetch | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
| `WEB_FETCH_RESOURCES_MEDIUM` | Medium cookies JSON | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
| `WEB_SEARCH_AGENTIC_THINKING_BUDGET` | Web search thinking budget | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
| `WEB_SEARCH_PRIMARY_BACKEND` | Primary web search backend | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
| `WEB_SEARCH_BACKEND` | Web search backend | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
| `WEB_SEARCH_HYBRID_MODE` | Hybrid mode (sequential|parallel) | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
| `WEB_SEARCH_SEGMENTER` | Search segmenter | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
| `OPEX_AGG_CRON` | Accounting aggregation schedule | `deployment/docker/all_in_one/sample_env/.env.backend` | worker |
| `BUNDLE_CLEANUP_ENABLED` | Enable bundle cleanup loop | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/worker |
| `BUNDLE_CLEANUP_INTERVAL_SECONDS` | Cleanup interval | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/worker |
| `BUNDLE_CLEANUP_LOCK_TTL_SECONDS` | Cleanup lock TTL | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/worker |
| `BUNDLE_REF_TTL_SECONDS` | Active bundle ref TTL | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/worker |
| `LOG_LEVEL` | Log level | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/kb/worker |
| `LOG_MAX_MB` | Log rotation size | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/kb/worker |
| `LOG_BACKUP_COUNT` | Log rotation count | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/kb/worker |
| `LOG_DIR` | Log directory | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/kb/worker |
| `LOG_FILE_PREFIX` | Log file prefix | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/kb/worker |
| `PY_CODE_EXEC_IMAGE` | Executor image name | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
| `PY_CODE_EXEC_TIMEOUT` | Executor timeout (seconds) | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
| `PY_CODE_EXEC_NETWORK_MODE` | Executor network mode | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
| `ACCOUNTING_SERVICES` | Accounting services JSON | `deployment/docker/all_in_one/sample_env/.env.backend` | chat/worker |
| `AUTH_TOKEN_COOKIE_NAME` | Auth token cookie name | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
| `ID_TOKEN_COOKIE_NAME` | ID token cookie name | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
| `MCP_CACHE_TTL_SECONDS` | MCP cache TTL | `deployment/docker/all_in_one/sample_env/.env.backend` | chat |
