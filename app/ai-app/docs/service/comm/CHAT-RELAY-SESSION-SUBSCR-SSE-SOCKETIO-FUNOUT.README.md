...
```
[SSEHub] register session=abc123 stream_id=xyz789 tenant=alldemo project=example-product-demo total_now=1
```
...
```
[ChatRelayCommunicator] acquire session=abc123 count_before=0 channel=alldemo:example-product-demo:chat.events.abc123
```
...
```
[ServiceCommunicator] subscribe_add self_id=... pubsub_id=... new=[kdcube.relay.chatbot.alldemo:example-product-demo:chat.events.abc123]
```
...
[sse_stream] relay diagnostic session=abc123 stream_id=xyz789
  ...
  subscribed_channels=[kdcube.relay.chatbot.alldemo:example-product-demo:chat.events.abc123]
  refcounts={'abc123': 1}
```
...
```
Publishing event 'chat_delta' to 'kdcube.relay.chatbot.alldemo:example-product-demo:chat.events.abc123'
