...
- Keep `control_plane.py` as the top‑level router only.
...
## Subscription Plans and Stripe Price Mapping

Stripe price IDs are mapped in `subscription_plans`:

- `plan_id` is the internal plan identifier used for quota policies.
- `stripe_price_id` is the Stripe price reference.

A Stripe subscription is created by referencing a plan_id that is marked as `provider=stripe`.
...
1. Admin calls `POST /subscriptions/create` with:
   ...
   - optional `stripe_price_id` override
...
- Request `cancel_at_period_end` in Stripe.
- The current period balance remains usable until period end.
- Record a pending internal event (`external_economics_events`, `source='internal'`, `kind='subscription_cancel'`).
...
1. Immediate local debit of lifetime credits.
...
- Create Stripe products/prices and store `stripe_price_id` in `subscription_plans`.
...
- Subscriptions are mapped to per‑period budgets and do not carry balances across months.
