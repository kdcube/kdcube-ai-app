...
import { inferLanguage } from '../../support/highlight.ts'
...
function DownloadsPanelImpl({
  ...
  const handleAttachmentDownload = async (attachment: TurnAttachment, index: number) => {
    try {
      ...
      if (objectRef) {
        vm.downloadFile(objectRef, attachment.name, attachment.mime ?? undefined)
        return
      }
  ...
  const handleFileDownload = async (file: FileArtifact) => {
    ...
    try {
      ...
      vm.downloadFile(objectRef, file.filename, file.mime ?? undefined)
    } catch (error) {
      onError(messageForError(error))
    } finally {
      setDownloadingId(null)
    ...
    <div className="flex flex-col gap-2 pt-1">
      {sortedArtifacts.map((artifact) => {
        ...
        if (artifact.kind === 'named_service_search') {
          ...
          return (
            <details
              key={`${artifact.kind}-${artifact.searchId}`}
              className="k-workitem k-namespace-tint"
              style={namespaceStyleVars(scope, namespaceStyles) as CSSProperties | undefined}
              open
            >
              <summary className="k-workitem-head">
                <span className="k-workitem-icon">
                    ...
                    {artifact.items.slice(0, 8).map((item, idx) => {
                      ...
                      return (
                        <button
                          ...
                          className="k-result-row k-namespace-row text-left"
                          style={contextChipStyle(context, namespaceStyles) as CSSProperties | undefined}
                          title={item.summary || label}
                          onClick={() => {
                            vm.openContextChip(context as Parameters<typeof vm.openContextChip>[0])
                          }}
                          onDragStart={(event) => {
                            if (!ref) return
  ...
  return (
    <div className="flex flex-col gap-2 pt-1">
      {events.map((event) => {
        ...
        return <ArtifactFeed key={event.key} artifacts={[event.artifact]} namespaceStyles={namespaceStyles} />
      })}
    </div>
  )
}
