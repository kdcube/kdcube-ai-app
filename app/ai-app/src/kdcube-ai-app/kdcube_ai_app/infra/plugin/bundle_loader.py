...
def _execute_in_bundle_venv(
    ...
    with tempfile.TemporaryDirectory(prefix="bundle-venv-", dir=str(_bundle_venv_dir(bundle_id))) as temp_dir:
        ...
        cmd = [
            str(venv_python),
            "-c",
            "from kdcube_ai_app.infra.plugin.bundle_loader import _bundle_venv_entry_main as _m; raise SystemExit(_m())",
            str(request_path),
            str(args_path),
            str(kwargs_path),
            str(response_path),
...
def _raise_if_singleton_base_workflow(target: Any, *, spec: BundleSpec) -> None:
...
def _resolve_module(spec: BundleSpec) -> types.ModuleType:
    key = _cache_key(spec)
    ...
    if p.is_file():
        if p.suffix in {".zip", ".whl"}:
            if not spec.module:
                raise ImportError("For .zip/.whl bundles you must provide 'module' (e.g., 'example_bundle').")
...
def clear_bundle_loader_caches() -> None:
...
# Backward-compatible public name. New code should use clear_bundle_loader_caches.
clear_agentic_caches = clear_bundle_loader_caches
...
def get_cached_manifest(spec: BundleSpec) -> "BundleInterfaceManifest | None":
...
def evict_spec(spec: BundleSpec, *, drop_sys_modules: bool = True) -> bool:
    """
...
def evict_bundle_scope(spec: BundleSpec, *, drop_sys_modules: bool = True) -> Dict[str, int]:
    """
