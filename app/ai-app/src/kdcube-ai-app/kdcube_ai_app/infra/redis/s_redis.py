...
# ── REDIS OVER SSH ─────────────────────────────────────────────────────────────
def _connect_redis(host: str, port: int, db: int, password: str) -> aioredis.Redis:
...
# ---- LIST KEYS ----
async def list_keys_by_pattern_via_ssh(
        pattern: str,
        *,
...
# ---- DELETE KEYS ----
async def delete_keys_by_pattern_via_ssh(pattern: str) -> int:
    """
    ...
    with ssh_local_forward(
        ...
        to_delete: List[str] = [key async for key in r.scan_iter(match=pattern, count=1000)]
        if not to_delete:
            print(f"No keys match: {pattern!r}")
            await r.aclose()
            return 0
        ...
        try:
            for i in range(0, len(to_delete), 1000):
                ...
                deleted += await r.unlink(*chunk)
        except ResponseError:
            for i in range(0, len(to_delete), 1000):
                chunk = to_delete[i:i+1000]
                deleted += await r.delete(*chunk)
...
async def _main():
    ...
    if op == "delete":
        ...
        await delete_keys_by_pattern_via_ssh(pattern)
    else:
        # list
        ...
        await list_keys_by_pattern_via_ssh(pattern, limit=limit_int, with_meta=with_meta, as_json=as_json)
