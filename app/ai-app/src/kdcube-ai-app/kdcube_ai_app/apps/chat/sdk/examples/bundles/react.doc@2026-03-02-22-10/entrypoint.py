...
class ReactWorkflow(BaseEntrypoint):
    """React.doc bundle — gate + ReAct solver with a single-root ks: knowledge space."""
        ...
        if root_raw:
            ...
            if derived_roots:
                ...
                if all(path.resolve() == first.resolve() for path in derived_roots[1:]):
                    ...
                    self.logger.log(
                        "[react.doc] legacy knowledge.*_root values do not resolve to one common ai-app root.",
                        "WARNING",
                    )
        if repo_root and not source_root:
            self.logger.log(
                "[react.doc] knowledge.root should point at the common ai-app root when knowledge.repo is set.",
                "WARNING",
            )
