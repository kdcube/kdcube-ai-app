...
# chat/sdk/tools/web/content_filters_fast.py
...
INPUT_DOC_FREE = """
- SEARCH OBJECTIVE: What the user is looking for
- SEARCH QUERIES USED: The queries that found these pages
- SOURCE PAGES: Web pages with SID numbers and full content
"""
# ----

# ---- PHASE 1 INPUT DOC
PHASE1_INPUT_DOC_JSON = """
- INPUT CONTEXT "objective" field - what the user searched for
- INPUT CONTEXT "queries" field - search queries used
- All SOURCES pages
"""

PHASE1_INPUT_DOC_FREE = """
- SEARCH OBJECTIVE - what the user searched for
- SEARCH QUERIES USED - search queries that were run
- All SOURCE PAGES with their SID numbers
"""
# ----

# ---- PHASE 2 INPUT DOC
PHASE2_INPUT_DOC_JSON = """
- Only the kept SIDs from Phase 1
- The full text of those pages
- DO NOT USE: objective or queries (they are irrelevant now)
"""

PHASE2_INPUT_DOC_FREE = """
- The kept SIDs from Phase 1
- The full content from those SOURCE PAGES
- SEARCH OBJECTIVE to guide extraction (for precision/balanced modes)
"""
# ----

def FILTER_AND_SEGMENT_HIGH_PRECISION(now_iso: str, input_mode: str = "json") -> str:

    if input_mode == "json":
        INPUT_DOC = INPUT_DOC_JSON
        PHASE1_INPUT_DOC = PHASE1_INPUT_DOC_JSON
        PHASE2_INPUT_DOC = PHASE2_INPUT_DOC_JSON
    else:
        INPUT_DOC = INPUT_DOC_FREE
        PHASE1_INPUT_DOC = PHASE1_INPUT_DOC_FREE
        PHASE2_INPUT_DOC = PHASE2_INPUT_DOC_FREE
...
- JSON: {{"phase1": [<sid>, ...], "phase2": {{"<sid>": [{{"s": "...", "e": "..."}}], ...}}}}
...
**Duplicate handling (Phase 1 is authoritative):**
- If two pages are near-duplicates (≥90% overlap), KEEP ONLY ONE SID.
- Prefer the more authoritative or more recent page; if unclear, keep the one with clearer structure.
...
Phase outputs:
□ Phase 1 outputs kept SIDs list
□ Phase 2 outputs spans only for kept SIDs
□ Phase 2 spans reflect objective-relevant content
...
def FILTER_AND_SEGMENT_HIGH_RECALL(now_iso: str, input_mode: str = "json") -> str:
...
def FILTER_AND_SEGMENT_BALANCED(now_iso: str, input_mode: str = "json") -> str:
    if input_mode == "json":
        INPUT_DOC = INPUT_DOC_JSON
        PHASE1_INPUT_DOC = PHASE1_INPUT_DOC_JSON
        PHASE2_INPUT_DOC = PHASE2_INPUT_DOC_JSON
...
If all pages dropped: {{"phase1": [], "phase2": {{}}}}
"""
