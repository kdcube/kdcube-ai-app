...
def test_multiline_internal_note_is_preserved_as_authored_with_tags():
    result = build_internal_note_compaction_result(
        blocks=[
            {
                "type": "react.note",
                "path": "conv:fi:turn_old.files/internal_notes/beacons.md",
                "text": "\n".join(
                    [
                        "[K] conv:fi:turn_old.files/report.html - source for rendered PDF",
    ...
    assert [b.get("path") for b in result.preserved_blocks] == [
        "conv:ar:turn_summary.react.note.preserved.1",
    ]
    meta = result.preserved_blocks[0].get("meta") or {}
    assert meta.get("source_path") == "conv:fi:turn_old.files/internal_notes/beacons.md"
...
def test_internal_note_cap_applies_across_note_blocks_not_tags():
    ...
    for idx, tag in enumerate(tags):
        blocks.append(
            {
                "type": "react.note",
                "path": f"conv:fi:turn_old.files/internal_notes/note-{idx}.md",
                "text": f"[{tag}] note {idx}",
                "meta": {"channel": "internal"},
            }
        )
