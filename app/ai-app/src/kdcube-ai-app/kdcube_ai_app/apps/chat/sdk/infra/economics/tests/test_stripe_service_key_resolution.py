...
"""
Tests that Stripe service classes resolve their API keys from explicit
arguments or already-loaded runtime settings.
...
class TestStripeSubscriptionServiceKeyResolution:
    def test_explicit_key_bypasses_service_secret(self, monkeypatch):
        secret_calls = []
        monkeypatch.setattr(stripe_mod, "_configured_stripe_secret",
                            lambda k: secret_calls.append(k) or "should-not-use")
    ...
    def test_configured_key_used_when_no_explicit_key(self, monkeypatch):
        monkeypatch.setattr(stripe_mod, "_configured_stripe_secret",
                            lambda k: "sk-settings-stripe" if k == "stripe.secret_key" else None)
        ...
        assert svc.stripe_api_key == "sk-settings-stripe"

    def test_global_key_used_as_fallback(self, monkeypatch):
        monkeypatch.setattr(stripe_mod, "_configured_stripe_secret",
                            lambda k: "sk-global-stripe" if k == "stripe.secret_key" else None)
    ...
    def test_none_when_no_key_configured(self, monkeypatch):
        monkeypatch.setattr(stripe_mod, "_configured_stripe_secret", lambda k: None)
...
class TestStripeEconomicsWebhookHandlerKeyResolution:
    def _make_handler(self, monkeypatch, service_secret_fn, *, webhook_secret=None):
        monkeypatch.setattr(stripe_mod, "_configured_stripe_secret", service_secret_fn)
    ...
    def test_none_webhook_secret_when_no_key(self, monkeypatch):
        handler = self._make_handler(monkeypatch, lambda k: None)
...
class TestStripeEconomicsAdminServiceKeyResolution:
    def test_bundle_key_used(self, monkeypatch):
        monkeypatch.setattr(stripe_mod, "_configured_stripe_secret",
                            lambda k: "sk-settings-admin" if k == "stripe.secret_key" else None)
        ...
        assert svc.stripe_api_key == "sk-settings-admin"
