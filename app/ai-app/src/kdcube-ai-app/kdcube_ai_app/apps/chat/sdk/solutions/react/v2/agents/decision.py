...
def build_decision_system_text(
    *,
    ...
    protocol = (
        ...
        "Skill causality rule: a skill catalog entry is only a summary. If you call react.read for sk:<skill_id>, stop after that action unless every other same-response action is independent of that skill's detailed content. Do not generate, write, render, code, search again, or call a domain tool based on the skill until the ACTIVE skill block is visible in a later round and you have reviewed it.\n"
