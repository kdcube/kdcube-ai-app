...
def _load_resolver_module():
    module_name = "_test_kdcube_copilot_knowledge_resolver"
    resolver_path = (
...
def _load_index_builder_module():
    module_name = "_test_kdcube_copilot_index_builder"
    index_builder_path = (
        pathlib.Path(__file__).resolve().parents[2]
        / "examples"
        / "bundles"
...
def test_kdcube_copilot_resolver_uses_bundle_storage_dir_when_knowledge_root_is_unset(tmp_path, monkeypatch):
...
def test_prepare_knowledge_space_replaces_stale_symlinks(tmp_path):
    ...
    stale_repo = tmp_path / "repos" / "kdcube-ai-app__kdcube.copilot.knowledge"
    fresh_repo = tmp_path / "repos" / "kdcube-ai-app__kdcube.copilot.knowledge__main"
...
def test_resolve_exec_namespace_allows_symlinked_source_subtree(tmp_path, monkeypatch):
    resolver = _load_resolver_module()
    knowledge_root = tmp_path / "bundle-storage" / "tenant" / "project" / "kdcube.copilot__main"
    repo_root = tmp_path / "bundle-storage" / "tenant" / "project" / "repos" / "kdcube.copilot.knowledge__main" / "app" / "ai-app"
    ...
    (knowledge_root / "src").symlink_to(pathlib.Path("../repos/kdcube.copilot.knowledge__main/app/ai-app/src"), target_is_directory=True)
