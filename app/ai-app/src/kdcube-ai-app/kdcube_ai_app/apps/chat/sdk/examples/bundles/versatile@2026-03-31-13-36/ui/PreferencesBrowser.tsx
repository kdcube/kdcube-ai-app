          ...
          <section style={{
            ...
            {report.items && report.items.length > 0 ? (
              <div style={{ marginTop: "16px" }}>
                <div style={{ fontWeight: 600, marginBottom: "8px" }}>Artifacts</div>
                <div style={{ display: "grid", gap: "8px" }}>
                  {report.items.map((item, index) => (
                    <div key={`${item.path || 'artifact'}-${index}`} style={{
                      padding: "12px 14px",
                      borderRadius: "16px",
                      background: "#f8faf8",
                      border: "1px solid rgba(24,35,29,0.08)",
                      ...
                      <div><strong>{item.path || 'artifact'}</strong></div>
                      <div style={{ opacity: 0.7, marginTop: "4px" }}>
                        {item.mime || 'unknown mime'}{item.size_bytes ? ` • ${item.size_bytes} bytes` : ''}
                      </div>
                    </div>
                  ))}
                </div>
