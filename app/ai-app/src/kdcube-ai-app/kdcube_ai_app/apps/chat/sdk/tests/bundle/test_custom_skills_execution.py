...
Run with:
  BUNDLE_UNDER_TEST=/abs/path/to/bundle pytest test_custom_skills_execution.py -v
  pytest test_custom_skills_execution.py --bundle-path=/abs/path/to/bundle -v
"""
...
def _skill_dirs_for_bundle(bundle_dir) -> list[tuple[pathlib.Path, str]]:
    ...
    try:
        ...
        if not result:
            pytest.skip(f"Bundle '{bundle_dir}' skills/ has no SKILL.md files")
        return result
...
class TestSkillInstructionLoading:
    ...
    def test_skill_md_body_is_non_empty_instruction(self, bundle, bundle_dir):
        """SKILL.md body (after frontmatter) is non-empty instruction text."""
        for skill_dir, _ in _skill_dirs_for_bundle(bundle_dir):
    ...
    def test_skill_body_contains_useful_content(self, bundle, bundle_dir):
...
class TestGetSkillHelper:
    ...
    def test_get_skill_resolves_registered_skill(self, bundle, bundle_dir):
        """get_skill() returns a SkillSpec for a skill that exists in the bundle."""
        dirs = _skill_dirs_for_bundle(bundle_dir)
        if not dirs:
    ...
    def test_skill_spec_has_id_and_namespace(self, bundle, bundle_dir):
