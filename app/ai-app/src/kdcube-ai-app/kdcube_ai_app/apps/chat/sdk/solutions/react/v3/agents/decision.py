...
from kdcube_ai_app.apps.chat.sdk.solutions.react.v3.layout import (
    build_tool_catalog,
    build_instruction_catalog_block,
)
from kdcube_ai_app.apps.chat.sdk.solutions.react.v3.call import get_react_tools_catalog
...
"""
    ...
    protocol = (
        ...
        "After </channel:code>, STOP. The platform calls you again if another round is needed.\n"
        "Do NOT emit a second full thinking/ReactDecisionOutV2/code triplet in the same response.\n"
        "In <channel:thinking>, write a brief user-facing status in markdown.\n"
        "The thinking channel is shown to the user. Keep it very short (1-2 sentences, no lists).\n\n"
    )
    if (multi_action_mode or "").strip().lower() == "safe_fanout":
        protocol += (
            "<channel:ReactDecisionOutV2> is the action channel. One channel instance means one action.\n"
            "If you want multiple actions in one round, repeat only <channel:ReactDecisionOutV2>. Do NOT repeat the full thinking/ReactDecisionOutV2/code triplet.\n"
            "Each <channel:ReactDecisionOutV2> instance must contain exactly one ```json fenced block with one ReactDecisionOutV2 object:\n"
            "```json\n"
            f"{json_hint}\n"
            "```\n"
            "Example:\n"
            "<channel:thinking>...one short status for the whole round...</channel:thinking>\n"
            ...
            "Use multi-action only when every action can be fully planned from the context already visible before the round starts.\n"
            "Execution is sequential and you do NOT review intermediate results in the middle, so action B must not depend on action A's result.\n"
            "Do NOT use exec_tools.execute_code_python in a multi-action round.\n"
            "If you use exec_tools.execute_code_python, it must be the only action in the round and <channel:code> belongs only to that exec round.\n"
            "Do NOT mix complete/exit with tool calls in a multi-action response.\n"
            "If you cite a channel token literally, write it in backticks like `channel:CHANNEL_ID`.\n\n"
        )
    else:
        protocol += (
            ...
            "For now we support only one action per round, so you must produce exactly one <channel:ReactDecisionOutV2> channel instance in the whole response.\n"
            ...
            f"{json_hint}\n"
            "```\n\n"
            "If multiple tools are needed, emit only the first action now. Never simulate round 2 yourself.\n"
            "Minimal valid shape:\n"
    ...
    protocol += (
        ...
        "When you need exec_tools.execute_code_python, you MUST write code in this channel.\n"
        "If the action is not exec_tools.execute_code_python, <channel:code> must be empty.\n"
        "Exec tool DOES NOT HAVE a code param. Code goes only in <channel:code>.\n"
    )
