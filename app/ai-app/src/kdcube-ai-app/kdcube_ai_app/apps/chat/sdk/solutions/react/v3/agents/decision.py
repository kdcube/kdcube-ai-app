    ...
    *,
    ...
    if _multi_action_enabled(multi_action_mode):
        protocol = (
            ...
            "<channel:action>```json {{ \"action\":\"call_tool\", \"tool_call\":{{\"tool_id\":\"rendering_tools.write_pptx\", \"params\":{{\"path\":\"turn_<current>/files/report/report.pptx\", \"content\":\"ref:conv:fi:conv_<conversation_id>.turn_<earlier>.files/report/report.md\"}}}} }} ```</channel:action>\n"
            ...
            "\n"
            "If you emit multiple actions in one round (after passing the multi-action gate above), use this shape — one <channel:action> per action:\n"
            "<channel:thinking>...short status for the round...</channel:thinking>\n"
            ...
            "Exec completeness: an exec_tools.execute_code_python action runs only when it has BOTH params.contract AND its Python in <channel:code> immediately after it; an incomplete exec does not run. Exec follows the strategy matrix like any other tool (its catalog traits decide compatibility); the independence gate still applies, so do not pair exec with an action that would consume its not-yet-visible output.\n"
            ...
            "complete/exit may share its round only with a NEUTRAL tool action (e.g. a memory write), emitted as a SEPARATE second <channel:action>. Pairing the close with a non-neutral tool — or embedding final_answer inside a tool's call_tool object — would claim the work is done before that tool's result exists.\n"
