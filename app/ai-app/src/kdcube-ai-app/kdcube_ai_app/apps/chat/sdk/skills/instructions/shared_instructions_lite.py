...
- The contract is the EXHAUSTIVE list of what is kept: the harness hosts ONLY the files listed in the contract; any file your code writes that is not contracted is discarded when the turn ends (your next turn starts EMPTY and can only PULL contracted files — an un-contracted file has no `fi:` ref and is gone for you, the user, and later steps). So contract EVERY file that could be useful on its own later, not just the final deliverable — intermediates included (mark reusable ones `visibility="internal"`).
- MOST COMMON MISTAKE: generating an Excel/PDF/DOCX that embeds charts/images and contracting ONLY the workbook. Embedding copies the image bytes into the document, but each standalone image is a SEPARATE output and is discarded unless contracted too — one entry per image. Same for datasets and intermediate exports.
