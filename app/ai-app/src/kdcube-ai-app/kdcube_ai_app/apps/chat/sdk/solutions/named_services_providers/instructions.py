...
The tool catalog is authoritative. A `named_services.*` tool may be used for the namespaces listed in that tool's `namespaces applicable` scope. Namespace discovery supplies domain semantics and optional scoped search namespaces.
...
2. Understand an unfamiliar namespace -> `named_services.provider_about(namespace=...)`. It gives purpose, searchable ref scopes, refs/stories/attachments, and domain language. For search, the `namespace` argument is the scope: a scoped namespace searches objects whose refs live in that scope.

3. Know exact object fields or search filters -> `named_services.object_schema(namespace=..., object_kind=... or object_ref=...)`. It gives the object body shape, filter contract, and concrete tool recipes when available.
