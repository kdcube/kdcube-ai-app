...
The widget reads/writes through the operations below. It shows identity links
...
## Identity vs delegated account contract
...
The current manual identity-link form is a development/onboarding fixture. A
...
| `identity_links_list` | GET | operations | List external identities linked to the current platform user. |
| `identity_link_upsert` | POST | operations | Link a verified external identity to the current platform user. User-facing route; does not grant roles. |
| `identity_link_remove` | POST | operations | Remove one external identity link from the current platform user. |
| `identity_link_challenge_create` | POST | operations | Create a short-lived one-time proof challenge for the current platform user. |
| `identity_link_challenge_status` | POST | operations | Read a proof challenge without claiming it. Platform-first challenges are limited to their platform user; provider-first pending challenges can be previewed by the currently signed-in platform user before explicit confirmation. |
...
```text
Platform-first:
  KDCube session creates challenge -> Telegram Mini App completes it.

Telegram-first:
  Telegram Mini App creates provider proof -> KDCube session claims it.
```

| Alias | Method | Route | Purpose |
| --- | --- | --- | --- |
| `telegram_identity_link_start` | POST | public | Validate Telegram Mini App `initData`, create a pending provider proof, and return `platform_claim_url`. |
| `telegram_identity_link_status` | POST | public | Validate Telegram Mini App `initData` and report whether that Telegram subject is already linked. |
| `telegram_identity_link_remove` | POST | public | Validate Telegram Mini App `initData` and unlink that Telegram subject from its platform user. |
| `telegram_identity_link_complete` | POST | public | Validate Telegram Mini App `initData` and complete a pending Telegram identity-link challenge. |
...
`X-KDCube-Auth-Authenticator-ID: <authenticator_id>`. `X-KDCube-Auth-Provider`,
`X-KDCube-Auth-Integration-ID`, and `X-KDCube-Auth-Connection-ID` are accepted
only as migration aliases for older callers. `telegram_identity_link_complete`
also requires body `{ "data": { "challenge_id": "..." } }`.
...
Connection Hub modules: they can read Connection Hub identity-link storage,
...
```json
{
  ...
  "telegram_link_url": "https://.../public/widgets/telegram_miniapp/link?link_challenge=one-time-token"
}
```
...
```http
POST /api/integrations/bundles/{tenant}/{project}/connection-hub@1-0/public/telegram_identity_link_start
X-Telegram-Init-Data: <Telegram.WebApp.initData>
...
`connection_hub.identity.link_changed` to it after
`identity_link_challenge_claim` succeeds.
...
```http
POST /api/integrations/bundles/{tenant}/{project}/connection-hub@1-0/operations/identity_link_challenge_claim
Content-Type: application/json
...
`identity_link_challenge_status` first, show the Telegram identity and current
...
```http
POST /api/integrations/bundles/{tenant}/{project}/connection-hub@1-0/public/telegram_identity_link_complete
X-Telegram-Init-Data: <Telegram.WebApp.initData>
Content-Type: application/json
