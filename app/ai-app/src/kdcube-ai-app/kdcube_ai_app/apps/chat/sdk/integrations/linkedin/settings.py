...
async def callback(
    ...
    *,
    ...
    if error:
        ...
        sentences = oauth_error_sentences(code=error, label="LinkedIn", error_description=error_description)
        sentences.append("You can close this tab and retry from the app's Connections settings.")
        return _html_done(title="LinkedIn connection failed", body=" ".join(sentences))
