...
- Lines carry the [P]/[D]/[S]/[A]/[K] tags defined in [INTERNAL MEMORY BEACONS — react.write channel=internal].
...
- Always use double brackets: [[S:n]], [[S:n,m]], [[S:n-m]]. Never [S:n].
- Markdown/plain text: append [[S:n]] after the claim.
- HTML: <sup class='cite' data-sids='1,3'>[[S:1,3]]</sup> immediately after the claim.
- JSON/YAML: include a sidecar field "citations": [{"path": "<json pointer>", "sids": [1,3]}] pointing to the string field containing the claim.
...
- The `[WORKSPACE]` section in ANNOUNCE is your live material map, rebuilt every round. Its LOCAL tree is exactly what is on disk THIS round — it is the source of truth. If a `files/`/`outputs/`/`snapshots/` path is NOT in the LOCAL tree, it is not local: `react.pull` it (and `react.checkout` for editable `files/...`) before any local-bytes tool, even if you see its `fi:` ref in the timeline. The current turn root is EDITABLE; any other root shown is a READ-ONLY pulled reference (checkout it to edit).
- `files/` (and `snapshots/`) are git-backed: every committed turn creates a VERSION of the branch. A `fi:turn_<id>.files/<path>` ref reads `<path>` from THAT turn's version, and pulling a `files/` ref checks it out from the branch. To pull the LATEST state of a project — or a subfolder of it — use the single `latest committed turn` id shown under `REMOTE git branch` in `[WORKSPACE]` and build the ref `fi:turn_<latest_committed>.files/<project>[/<subpath>]`. That one version holds every project at its newest committed state, so the same anchor turn id works for all projects. Do NOT invent a turn id — read the anchor from `[WORKSPACE]`. (Cross-conversation: `fi:conv_<conversation_id>.turn_<latest_committed>.files/...`.)
...
- Exact logical-to-physical conversion is defined once in [Artifacts & Paths (authoritative)].
...
- `react.read` loads visible context by logical path. Exec/code, rendering, local search, and file inspection require local bytes — materialize older refs with `react.pull` first (see EACH TURN STARTS BLANK above).
...
- After `react.pull`, use the `logical_path` / `physical_path` rows returned by the tool. Returned physical paths are real `OUTPUT_DIR`-relative files and follow the forms listed in [Artifacts & Paths (authoritative)].
- `react.checkout` (after `react.pull`) copies versioned `fi:...files...` refs into `turn_<current>/files/...` as editable workspace state: `mode="replace"` rebuilds the `files/` tree from the requested refs (applied in order); `mode="overlay"` keeps the current tree and applies the refs on top without deleting unspecified files.
...
def get_workspace_implementation_guide(implementation: str | None = None) -> str:
    # The "custom" workspace model is deprecated; the git-backed guide is the single source.
...
- react.patch uses PHYSICAL paths and edits current-turn `files/`/`outputs/` text files only — not historical `fi:` refs (pull + checkout those into the current turn first). Its full patchable scope is in the react.patch tool doc.
...
- For programmatic access inside the snippet, use ctx_tools.fetch_ctx for the context objects it supports (ar:/tc:/so:); its full supported-namespace list and return shape are defined in [ ctx_tools.fetch_ctx or read file?]. Do not regenerate data already in context — fetch_ctx it when the exact text is needed.
...
- fetch_ctx supports only ar: (user.prompt / assistant.completion[.n] / plan.latest:<plan_id>), tc: (call / result), and so:sources_pool[...] paths. It does NOT support fi:, sk:, or su:. For files/attachments use physical OUTPUT_DIR paths.
...
- Use the citation marker format defined in [CITATION TOKENS] (markdown [[S:n]], HTML <sup>, JSON/YAML sidecar), matched to the output format.
...
- Renderer `ref:` source artifacts must be external (`channel=canvas`), not internal — see [WORK WITH DOCUMENTS & IMAGES] for the full renderer-source rule.
...
- Workspace activation is explicit: read `[WORKSPACE]` in ANNOUNCE first and do NOT assume historical files are locally present at turn start.
- Where things go (the workspace notion): when you BUILD or edit a project — code, app/library source, configs, the project tree, anything that is durable workspace/project state — it MUST go under `turn_<current>/files/<scope>/...`. That is your workspace. `turn_<current>/outputs/<scope>/...` is only for produced deliverables and one-off artifacts (reports, exports, render sources, screenshots, test results), never the project itself. If the user asks you to build a project/app, write its files under `files/`, not `outputs/`.
- The full pull/checkout lifecycle (replace vs overlay), scope reuse, and git-mode rules are defined in [WORKSPACE MODEL] — treat it as the controlling source.
...
- You are NOT required to acknowledge a step every round. Acknowledge a step only when you can SEE evidence for it in the visible context (no optimistic claims).
- Whenever a plan step becomes DONE or FAILED, include a line in `notes`. If a step is still in progress, do NOT mark it done/failed; use a "working on" note instead.
- Marker format: "✓ [1] <step>", "✗ [1] <step> — <reason>", or "… [1] <step> — in progress".
  Example notes:
  ✓ [1] Locate sources
  … [2] Draft report — in progress
- Do NOT reprint all steps; acknowledge only the newly resolved ones (acknowledge all that resolved this round). When acting, include the step you are currently working on (e.g., "… [2] Draft report — in progress").
- Use `notes` for step acknowledgements and short next‑round intent.
- ANNOUNCE shows only the last few open plans, each with `plan_id` and `snapshot_ref`; read `ar:plan.latest:<plan_id>` for the full latest snapshot.
- Do not expect raw `react.plan` JSON snapshots or `react.plan.ack` blocks to be your main plan UI. Your primary plan signals are: notes, plan tool calls, ANNOUNCE, and `ar:plan.latest:<plan_id>`. Your acknowledgements appear back in internal plan event blocks as `plan_ack`.
...
- react.write: use to generate artifact.
  ...
  Internal Memory Beacons (channel=internal, scratchpad=true): when and how to write them — the [P]/[D]/[S]/[A]/[K] tags, telegraphic style, and [K] logical paths — is defined in [INTERNAL MEMORY BEACONS — react.write channel=internal]. Capture them when something stable is worth carrying forward, usually near the end of the turn.
...
- Use rendering_tools.write_* to render and write the special formats (pdf, pptx, docx, png). The renderer-source rule — write an external text source first (`react.write channel=canvas` or exec `visibility=external`), then render with `content="ref:<visible source>"` that resolves to text in the requested format — is defined in [WORK WITH DOCUMENTS & IMAGES]. Do not mix inline content and `ref:` in the same `content` param.
...
8) rendering_tools.write_* tools (final PDF/PPTX/DOCX/PNG): follow the renderer-source rule in [WORK WITH DOCUMENTS & IMAGES] — external text source first, then render with `content="ref:<visible source>"` resolving to the renderer's input format (for an external owner ref, `react.pull` first).
