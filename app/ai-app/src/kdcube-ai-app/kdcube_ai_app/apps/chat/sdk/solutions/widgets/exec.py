...
class CodegenChanneledStreamingWidget:
    ...
    def __init__(
        ...
        *,
        ...
        self.pending_program_name: Optional[str] = None
        self.current_status: Optional[str] = None
        self.event_log: List[dict] = []
        self.captured_code: str = ""
        self.contract_emitted = False
        self.program_emitted = False
        self.pending_contract: Optional[dict] = None
        self.pending_code_prefix: str = ""
    ...
    async def capture_params(self, params: dict) -> None:
        ...
        if prog_name:
            self.pending_program_name = prog_name
            if self.activated and not self.program_emitted:
    ...
    async def _try_parse_params(self, *, final: bool = False) -> None:
        ...
        if self.tool_id_value and tool_id != self.tool_id_value and not self.captured_code:
            if final:
                self.params_parsed = True
            return
        if self.tool_id_value and tool_id == self.tool_id_value:
            await self._activate(reason="tool_id")
    ...
    async def _activate(self, *, reason: str) -> None:
        if self.activated:
            return
        self._log_event("activate", reason=reason)
        self.activated = True
        # Only emit status now (once) when we know this is exec or we see code.
        await self.send_status(status="gen")
        if self.pending_program_name and not self.program_emitted:
            await self.emit_program_name(self.pending_program_name)
        if self.pending_contract and not self.contract_emitted:
            await self.emit_contract(self.pending_contract)
    ...
    async def _emit_chunk(self, text: str) -> None:
        ...
        if not self.activated:
            await self._activate(reason="code")
        elif self.current_status != "gen":
            await self.send_status(status="gen")
        await self.emit_delta(
            text=text,
    ...
    async def feed_raw(self, chunk: str) -> None:
        ...
        if not self.activated and not chunk.strip():
            self.pending_code_prefix += chunk
            return
        if not self.activated and self.pending_code_prefix:
            chunk = self.pending_code_prefix + chunk
            self.pending_code_prefix = ""
