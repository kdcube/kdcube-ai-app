...
1. Fetch the Tier 1 pack in order (navigate first, then the rest):
   ...
   - `repo:kdcube-ai-app/app/ai-app/docs/sdk/bundle/build/how-to-assemble-bundle-with-sdk-building-blocks-README.md` — SDK/platform building-block map; route here **before implementing any subsystem** (Telegram, Email, Delivery, Tasks, web/browser/rendering/exec tools, widgets, jobs, MCP, Claude Code). Telegram / webhooks / Mini App / OAuth callbacks is one worked example of this routing — use the Telegram SDK (`repo:kdcube-ai-app/app/ai-app/docs/sdk/integrations/telegram/telegram-README.md` + `telegram-external-prereq-README.md`) and `repo:kdcube-ai-app/app/ai-app/docs/service/cicd/ngrok-README.md` for the local public-HTTPS recipe. The same routing applies to every other subsystem the bundle touches (including integrations not named here); when in doubt, run the SDK-discovery gate from `bundle-builder` Rule #6 instead of hand-rolling
