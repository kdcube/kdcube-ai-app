...
def _load_skills_descriptor(bundle):
...
class TestSkillsDescriptorStructure:
    ...
    def test_skills_descriptor_defines_custom_skills_root(self, bundle):
        """skills_descriptor.py defines CUSTOM_SKILLS_ROOT attribute."""
        mod, _ = _load_skills_descriptor(bundle)
        assert hasattr(mod, "CUSTOM_SKILLS_ROOT"), (
            "skills_descriptor must define CUSTOM_SKILLS_ROOT"
        )

    def test_custom_skills_root_is_path_or_none(self, bundle):
...
class TestSkillsSubsystem:
    ...
    def test_bundle_skills_loaded_when_skills_dir_exists(self, bundle):
        """When bundle has skills/ directory, at least one skill is registered."""
        mod, bundle_dir = _load_skills_descriptor(bundle)
