...
async def solver_codegen_stream(
    ...
    sys = (
        ...
        "- Store OUTPUT_DIR-relative paths in slots.\n"
        "\n"
        "## Live Progress & Checkpoints (CRITICAL)\n"
        "- The runtime injects `set_progress(...)`, `done()`, and `fail(...)` globals.\n"
        "- **Call `set_progress(...)` after each milestone** and whenever any slot becomes available (even partially).\n"
        "- Use `flush=True` only at step boundaries to persist a checkpoint to `result.json` immediately.\n"
        "- If the program is terminated (timeout/kill), the runtime writes a **partial** `result.json` automatically from the last `set_progress(...)` state.\n"
        "\n"
        ...
        "    set_progress(objective=objective, story_append=\"Started. Planned gather → generate → render.\", flush=True)\n"
        "\n"
        "    try:\n"
        "        set_progress(story_append=\"Searching sources.\")\n"
        ...
        "        set_progress(story_append=f\"Found {len(sources)} sources.\", flush=True)\n"
        "\n"
        "        set_progress(story_append=\"Generating draft from sources.\")\n"
        ...
        "        )\n"
        "\n"
        "        set_progress(story_append=\"Rendering PDF.\")\n"
