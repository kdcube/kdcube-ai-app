...
from kdcube_ai_app.apps.chat.sdk.runtime.solution.react.v2.layout import build_instruction_catalog_block
...
ProjectModeV2 = Literal[
    "plan_breakdown",
    "stage_zoom_in",
    "integration_zoom_out",
    "single_deliverable",
]
TurnScopeV2 = Literal["new_request", "continue_request"]
NextStepV2 = Literal["react_loop", "llm_only", "clarification_only"]


class CoordinatorPolicyV2(BaseModel):
    turn_scope: TurnScopeV2 = "new_request"
    project_mode: ProjectModeV2 = "plan_breakdown"
    scope_notes: str = ""
...
class CoordinatorDecisionV2(BaseModel):
...
class UnifiedCoordinatorOutV2(BaseModel):
    policy: CoordinatorPolicyV2 = Field(default_factory=CoordinatorPolicyV2)
    decision: CoordinatorDecisionV2 = Field(default_factory=CoordinatorDecisionV2)
...
def _get_2section_protocol(json_shape_hint: str) -> str:
    return (
        ...
        "CHANNEL 2 — STRUCTURED JSON CHANNEL (UnifiedCoordinatorOutV2 decision):\n"
        "Marker:\n"
...
async def coordinator_planner_stream_v2(
    ...
    *,
    ...
    sys_1 = (
        ...
        "• decision (plan, budgets, next_step, instructions)\n"
        ...
        "Clarify ONLY when the target document/turn is ambiguous.\n"
        "\n"
        ...
        "\n"
        "[TURN FEASIBILITY PRINCIPLE]\n"
        "Plan only the next feasible slice (this turn). Use 2–5 short plan steps when mode=new|update.\n"
        "Downstream will render plan steps as checkboxes (□/✓/✗/…) and compute turn outcome from them.\n"
        ...
        "If mode=update, emit updated steps and note in instructions_for_downstream that the plan was updated.\n"
        "\n"
        "[BUDGETS]\n"
        "Provide total budgets for THIS TURN only: exploration + exploitation.\n"
        "Do NOT include render/infra.show in budget accounting.\n"
        "Keep numbers small and realistic (0–3 typical).\n"
        "\n"
        "[OUTPUT ARTIFACTS]\n"
        "If final artifacts are required, mention them explicitly in plan steps and/or instructions_for_downstream.\n"
        "\n"
        "[TURN MEMORIES & FEEDBACK]\n"
        "• In the context, [TURN MEMORIES — CHRONOLOGICAL (newest→oldest)] lists recent per-turn memories (preferences/signals).\n"
        "  Use them when planning or refining the objective; newer items override older ones on conflicts.\n"
        "• [USER FEEDBACK — CHRONOLOGICAL (newest→oldest)] lists feedback signals that may alter quality/acceptance; incorporate them into your plan.\n"
        "\n"
    ...
    json_hint = (
        ...
        "    \"scope_notes\": \"surface level; focused scope\"\n"
        "  },\n"
        "  \"decision\": {\n"
        "    \"plan\": {\n"
    ...
    async def _call_coordinator(*, blocks):
        ...
        log.info(f"[COORDINATOR.v2]. System message (1):\n{sys_1}")
        log.info(f"[COORDINATOR.v2]. System message (2):\n{sys_2}")
        log.info(f"[COORDINATOR.v2]. System message (3):\n{sys_3}")
        log.info(f"[COORDINATOR.v2]. User message:\n{msg_blocks}")
        return await _stream(
            svc,
            client_name="solver.unified-planner.v2",
            client_role="solver.unified-planner.v2",
