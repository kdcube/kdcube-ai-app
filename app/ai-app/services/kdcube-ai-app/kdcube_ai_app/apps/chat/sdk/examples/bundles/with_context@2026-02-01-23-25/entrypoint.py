...
from .orchestrator.workflow import MinimalContextWorkflow

BUNDLE_ID = "example.with_context"


@agentic_workflow(name=BUNDLE_ID, version="1.0.0", priority=100)
class WithContextWorkflow(BaseEntrypoint):
    """Minimal bundle with context search + ctx.reconciler + simple answer."""
    ...
    def _build_graph(self) -> StateGraph:
        ...
        async def orchestrate(state: BundleState) -> BundleState:
            ...
            from kdcube_ai_app.apps.chat.sdk.context.graph.graph_ctx import GraphCtx, NullGraphCtx
            ...
            graph = GraphCtx() if self.settings.APP_GRAPH_ENABLED else NullGraphCtx()
            ...
            await conv_idx.init()
            await kb.init()
            if getattr(graph, "enabled", False):
