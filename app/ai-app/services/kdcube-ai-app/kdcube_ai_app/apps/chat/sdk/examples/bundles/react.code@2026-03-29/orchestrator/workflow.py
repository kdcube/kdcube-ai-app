...
# Core orchestration logic for the react.code bundle.
# Same gate + solver pipeline as react.doc, with CodeGraphClient injected.
...
class WithReactCodeWorkflow(BaseWorkflow):
    def __init__(
        self,
        *,
        conv_idx: ConvIndex,
        ...
        try:
            async def _gate_node(state: Dict[str, Any]) -> Dict[str, Any]:
                ...
                logging_helpers.log_agent_packet(
                    "gate", "gate",
                    {"user_thinking": gate_channels.get("thinking"), "agent_response": gate_payload},
                )
            ...
            async def _react_node(state: Dict[str, Any]) -> Dict[str, Any]:
                ...
                await self._emit_turn_work_status(
                    ["closing the loop", "wrapping up", "tying it together"]
                )
