...
# Identical to react.doc — shared knowledge indexing logic.
...
def build_knowledge_index(
    *,
    ...
    if src_root:
        md_lines.append("- Use `react.read([\"ks:src/<path>\"])` to open source files referenced by docs.")
    if deploy_root:
        md_lines.append("- Use `react.search_knowledge(query=..., root=\"ks:deploy\")` to search deployment docs.")
        md_lines.append("- Use `react.read([\"ks:deploy/<path>\"])` to open deployment files.")
    if tests_root:
        md_lines.append("- Use `react.read([\"ks:tests/<path>\"])` to open bundle test fixtures.")
    md_lines += ["", "## Docs"]
    for item in entries:
...
def prepare_knowledge_space(
    *,
    ...
    if src_root and src_root.exists():
        _safe_symlink(src_root, knowledge_root / "src")
