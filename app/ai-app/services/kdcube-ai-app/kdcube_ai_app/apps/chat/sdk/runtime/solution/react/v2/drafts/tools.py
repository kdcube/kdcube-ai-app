...
from kdcube_ai_app.apps.chat.sdk.runtime.solution.react.v2.context import ReactContext
from kdcube_ai_app.apps.chat.sdk.runtime.solution.react.v2.tools import (
    _tool_call_block,
    _run_post_patch_check,
)
...
async def handle_react_apply_patch(*, react: Any, state: Dict[str, Any], tool_call_id: str) -> Dict[str, Any]:
    ...
    turn_id = (context.turn_id or "").strip()
    ...
    for rel_path in [*result.summary.added, *result.summary.modified]:
        ...
        ok, msg = _run_post_patch_check(target_path)
        if not ok:
            notes.append({"path": str(target_path), "detail": msg})

    if notes:
        context.add_event(kind="note", data={
            "reason": "post_patch_check_failed",
            "tool_id": tool_id,
            "tool_call_id": tool_call_id,
            "notes": notes,
        })

    context.add_event_block({
        "turn": turn_id,
